...
#slide[
  ...
  3. Kalman filter for tracking health index for each sesnor residual.
    - Using N simple constant velocity models with each element of  of $bold(r)_k$ as outputs.
  $ bold(x)_k = vec(x_1, x_2) = vec("Health (residual)", "Degradation rate") quad A = mat(1, Delta t; 0, 1) $

]

#slide[
  = Visualisation of the filtered health state
  #figure(caption: "Filtered health state")[
    #image("residual_event.png")
  ]
]

#slide[
  = Visualisation of health slope
  #figure(caption: "Filtered health slope")[
    #image("slope.png")
  ]
]
...
#slide[
  ...
  How to get from these health states $bold(h)_k$ and health slopes $dot(bold(h))_k$ to predicted events?
  Physical Approach:
  - Define threshold $h_"thresh"$ where we need repairs.
  - Calculate time to failure $t_("fail") = (h_"thresh" - h_k)/dot(h)$
  - Issues:
    - $h_"tresh"$ is not a simple constant. It changes repair to repair.
    - $dot(h)$ is quite noisy.
    - We have N (for each sensor) health indices and slopes $bold(h), bold(dot(h))$ are vectors. We have to figure out a way to fuse them.
]

#slide[
  = Predicting service events
  Machine Learning Approach:
  - Use Kalman Filtered health states $bold(h)_k$ and health slopes $dot(bold(h))_k$ as features.
  - Train a model to predict time to events $t_("event")$ such that
    $ f(bold(h)_k, dot(bold(h))_k) = t_("event") $
  - Only small amount of quality features so it is feasible to train a model.
]

#slide[
  = Results of Machine Learning Approach
  #figure(caption: "Results of Machine Learning Approach")[
    #image("norul_xgb.png")
  ]
]
...
#slide[
  = Missed information
  - We can still improve this model by using more information.
  - Let's look at an example.
    - At t=100 we predict event in 100 steps.
    - At t=90 we predicted event in 105 steps.
    - We can combine these predictions to get better prediction.
    - This is implemented by a smoothing kalman filter over the predicted times.
]

#slide[
  = Final smoothened prediction
  #figure(caption: "Final smoothened prediction")[
    #image("rul.png")
  ]
]
...
#slide[
  = Final results
  #grid(columns: (1fr, 1fr))[
    - Baseline tree model: 93.60
    - Kalman filter model (without smoothing): 62.85
    - Kalman filter model (with smoothing): 45.23
    - Final model (with optimized hyperparams): 37.31
  ][
    #figure(caption: "Data challenge leaderboard")[
      #image("leader2.png")
    ]
  ]
]
